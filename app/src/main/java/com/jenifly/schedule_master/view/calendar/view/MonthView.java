...
public class MonthView extends CalendarView {
    ...
    public MonthView(Context context, DateTime dateTime, OnClickMonthViewListener onClickMonthViewListener) {
        ...
        //0周日，1周一
        DateUtils.NCalendar nCalendar2 = DateUtils.getMonthCalendar2(dateTime, Attrs.firstDayOfWeek);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mRowNum; i++) {
            for (int j = 0; j < 7; j++) {
                ...
                //当月和上下月的颜色不同
                if (DateUtils.isEqualsMonth(dateTime, mInitialDateTime)) {
                    //当天和选中的日期不绘制农历
                    if (DateUtils.isToday(dateTime)) {
                     ...
                     //   mSorlarPaint.setColor(mHollowCircleColor);
                        mSorlarPaint.setStrokeWidth(mHollowCircleStroke);
    ...
    public int getDrawHeight() {
        return (int) (getMonthHeight() - ScreenUtils.dp2px(getContext(), 10));
    }
        ...
        public boolean onSingleTapUp(MotionEvent e) {
            for (int i = 0; i < mRectList.size(); i++) {
                ...
                if (rect.contains((int) e.getX(), (int) e.getY())) {
                    ...
                    if (DateUtils.isLastMonth(selectDateTime, mInitialDateTime)) {
                        mOnClickMonthViewListener.onClickLastMonth(selectDateTime);
                    } else if (DateUtils.isNextMonth(selectDateTime, mInitialDateTime)) {
